...
# Sanity checks before you finalize a file
  ...
  - Initial pile counts (DRAW included) sum to exactly the total deck size -- \
    ...
    changed, added, removed, or resized any pile compared to the previous version, \
    re-verify the sum from scratch; a change to one pile's count is not safe on its \
    own unless you've also adjusted another pile to compensate.
